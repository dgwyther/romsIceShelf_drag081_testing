...
!
     ...
     &                 SetParAccess = .FALSE.)
        IF (exit_flag.ne.NoError) RETURN
!
!  Define turbulent heat exchange velocity
!
        IF (Hout(idgamt,ng)) THEN
          Vinfo( 1)=Vname(1,idgamt)
          Vinfo( 2)=Vname(2,idgamt)
          Vinfo( 3)=Vname(3,idgamt)
          Vinfo(14)=Vname(4,idgamt)
          Vinfo(16)=Vname(1,idgamt)
          Vinfo(22)='coordinates'
          Aval(5)=REAL(Iinfo(1,idgamt,ng),r8)
          status=def_var(ng, iNLM, HIS(ng)%ncid, HIS(ng)%Vid(idgamt),   &
     &                   NF_FOUT, nvd3, t2dgrd, Aval, Vinfo, ncname,    &
     &                   SetFillVal = .FALSE.)
          IF (exit_flag.ne.NoError) RETURN
        END IF
!
!  Define turbulent salt exchange velocity
!
        IF (Hout(idgams,ng)) THEN
          Vinfo( 1)=Vname(1,idgams)
          Vinfo( 2)=Vname(2,idgams)
          Vinfo( 3)=Vname(3,idgams)
          Vinfo(14)=Vname(4,idgams)
          Vinfo(16)=Vname(1,idgams)
          Vinfo(22)='coordinates'
          Aval(5)=REAL(Iinfo(1,idgams,ng),r8)
          status=def_var(ng, iNLM, HIS(ng)%ncid, HIS(ng)%Vid(idgams),   &
     &                   NF_FOUT, nvd3, t2dgrd, Aval, Vinfo, ncname,    &
     &                   SetFillVal = .FALSE.)
          IF (exit_flag.ne.NoError) RETURN
        END IF
!
!  Define temp in viscous sublayer
!
        IF (Hout(idtbvs,ng)) THEN
          Vinfo( 1)=Vname(1,idtbvs)
          Vinfo( 2)=Vname(2,idtbvs)
          Vinfo( 3)=Vname(3,idtbvs)
          Vinfo(14)=Vname(4,idtbvs)
          Vinfo(16)=Vname(1,idtbvs)
          Vinfo(22)='coordinates'
          Aval(5)=REAL(Iinfo(1,idtbvs,ng),r8)
          status=def_var(ng, iNLM, HIS(ng)%ncid, HIS(ng)%Vid(idtbvs),   &
     &                   NF_FOUT, nvd3, t2dgrd, Aval, Vinfo, ncname,    &
     &                   SetFillVal = .FALSE.)
          IF (exit_flag.ne.NoError) RETURN
        END IF
!
!  Define salinity in viscous sublayer.
!
        IF (Hout(idsbvs,ng)) THEN
          Vinfo( 1)=Vname(1,idsbvs)
          Vinfo( 2)=Vname(2,idsbvs)
          Vinfo( 3)=Vname(3,idsbvs)
          Vinfo(14)=Vname(4,idsbvs)
          Vinfo(16)=Vname(1,idsbvs)
          Vinfo(22)='coordinates'
          Aval(5)=REAL(Iinfo(1,idsbvs,ng),r8)
          status=def_var(ng, iNLM, HIS(ng)%ncid, HIS(ng)%Vid(idsbvs),   &
     &                   NF_FOUT, nvd3, t2dgrd, Aval, Vinfo, ncname,    &
     &                   SetFillVal = .FALSE.)
          IF (exit_flag.ne.NoError) RETURN
        END IF
