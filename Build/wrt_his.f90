...
!
     ...
     &                       ICESHELFVAR(ng) % gammaT)
          IF (status.ne.nf90_noerr) THEN
            IF (Master) THEN
              WRITE (stdout,10) TRIM(Vname(1,idgamt)), HIS(ng)%Rindex
            END IF
            exit_flag=3
            ioerror=status
            RETURN
...
!
     ...
     &                       ICESHELFVAR(ng) % gammaS)
          IF (status.ne.nf90_noerr) THEN
            IF (Master) THEN
              WRITE (stdout,10) TRIM(Vname(1,idgams)), HIS(ng)%Rindex
            END IF
            exit_flag=3
            ioerror=status
            RETURN
         ...
         END IF
!
! Write out viscous sublayer temp
!
     ...
     &                       ICESHELFVAR(ng) % Tb)
          IF (status.ne.nf90_noerr) THEN
            IF (Master) THEN
              WRITE (stdout,10) TRIM(Vname(1,idtbvs)), HIS(ng)%Rindex
            END IF
            exit_flag=3
            ioerror=status
            RETURN
...
!
     ...
     &                       ICESHELFVAR(ng) % Sb)
          IF (status.ne.nf90_noerr) THEN
            IF (Master) THEN
              WRITE (stdout,10) TRIM(Vname(1,idsbvs)), HIS(ng)%Rindex
            END IF
            exit_flag=3
            ioerror=status
            RETURN
