...
!
     ...
     &                                        'Must between 1 and 4'
            ...
            CASE ('Hout(idgamt)')
              IF (idgamt.eq.0) THEN
                IF (Master) WRITE (out,280) 'idgamt'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idgamt,:))
            CASE ('Hout(idgams)')
              IF (idgams.eq.0) THEN
                IF (Master) WRITE (out,280) 'idgams'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idgams,:))
            CASE ('Hout(idtbvs)')
              IF (idtbvs.eq.0) THEN
                IF (Master) WRITE (out,280) 'idtbvs'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idtbvs,:))
            CASE ('Hout(idsbvs)')
              IF (idsbvs.eq.0) THEN
                IF (Master) WRITE (out,280) 'idsbvs'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idsbvs,:))
            CASE ('Hout(idismr)')
              IF (idismr.eq.0) THEN
...
!
     ...
     &          '                     no-slip (-1.0).'
          IF (Hout(idgamt,ng)) WRITE (out,170) Hout(idgamt,ng),         &
     &       'Hout(idgamt)',                                            &
     &       'Write out turbulent heat exchange.'
          IF (Hout(idgams,ng)) WRITE (out,170) Hout(idgams,ng),         &
     &       'Hout(idgams)',                                            &
     &       'Write out turbulent salt exchange.'
          IF (Hout(idtbvs,ng)) WRITE (out,170) Hout(idtbvs,ng),         &
     &       'Hout(idtbvs)',                                            &
     &       'Write out viscous sublayer temperature.'
          IF (Hout(idsbvs,ng)) WRITE (out,170) Hout(idsbvs,ng),         &
     &       'Hout(idsbvs)',                                            &
     &       'Write out viscous sublayer salinity.'
